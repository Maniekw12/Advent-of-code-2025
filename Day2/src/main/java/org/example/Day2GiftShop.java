package org.example;


public class Day2GiftShop {
    public static void main(String[] args) {
        String input = "975119-1004307,35171358-35313940,6258659887-6258804661,737649-837633,85745820-85956280,9627354-9679470,2327309144-2327509647,301168-351545,537261-631588,364281214-364453549,9563727253-9563879587,3680-9127,388369417-388406569,6677501-6752949,650804-678722,3314531-3365076,1052-2547,24134-68316,8888820274-8888998305,82614-107458,456819-529037,358216-389777,24222539-24266446,874565-916752,3886244-3960191,25-110,9696951376-9696996784,171-671,5656545867-5656605587,75795017-75865731,1-16,181025-232078";

        long sum = solve(input);
        System.out.println("Sum of invalid IDs: " + sum);
    }

    public static long solve(String input) {
        long total = 0;
        String[] ranges = input.split(",");

        for(String range : ranges) {
            String[] parts = range.split("-");
            long start = Long.parseLong(parts[0]);
            long end   = Long.parseLong(parts[1]);

            for(long id = start; id <= end; id++) {
                if (isDoublePattern(id)) {
                    total += id;
                }
            }


        }
        return total;
    }

    public  static boolean isDoublePattern(long number){
        String s = String.valueOf(number);
        int len = s.length();

        if( len % 2 != 0) return false;
        String half = s.substring(0, len / 2);
        return s.equals(half + half);

    }

}
